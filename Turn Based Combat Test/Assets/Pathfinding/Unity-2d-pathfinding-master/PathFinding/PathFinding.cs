...
namespace PathFind
{
    ...
    */
    public class Pathfinding
    {
        ...
        {
            if(startPos.x >= grid.nodes.GetLength(0) || startPos.y >= grid.nodes.GetLength(1) || targetPos.x >= grid.nodes.GetLength(0) || targetPos.y >= grid.nodes.GetLength(1))// || grid.nodes[targetPos.x, targetPos.y]. == 0f)
            {
                return null;
            }
